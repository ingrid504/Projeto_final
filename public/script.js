...
function loginUser (event) {
    ...
    .catch((error) => {
        console.error(error);
        errorMessage.textContent = "Usuário ou senha inválidos. Tente novamente."; // Exibe mensagem de erro
    });
}   
...
function registerUser (event) {
    ...
    const username = document.getElementById("username").value;
    const password = document.getElementById("password").value;
    const errorMessage = document.getElementById("error-message"); // Elemento para exibir erros
    ...
    .then((data) => {
        // Armazenar informações no armazenamento local
        localStorage.setItem("username", username);
        localStorage.setItem("password", password); // Cuidado ao armazenar senhas em texto simples

        alert(data.message);
        window.location.href = "/public/index.html"; // Redireciona para a página inicial
    })
    .catch((error) => {
        console.error(error);
        errorMessage.textContent = "Erro ao cadastrar o usuário. Tente novamente."; // Exibe mensagem de erro
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (path.includes("index.html") && document.getElementById("loginForm")) {
        document.getElementById("loginForm").addEventListener("submit", loginUser);
    }

    if (path.includes("register.html") && document.getElementById("registerForm")) {
        document.getElementById("registerForm").addEventListener("submit", registerUser);
    }
    ...
    if (path.includes("agendar_visita.html") && document.getElementById("agendarForm")) {
        document.getElementById("agendarForm").addEventListener("submit", submitAgendamento);
    }
});
